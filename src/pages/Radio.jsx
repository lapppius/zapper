...
function getRadioInfoPromise(res) {
  return new Promise((resolve, reject) => {
    ...
    for (const claim of claims) {
      try {
        const expression = res.claims[claim][0].mainsnak.datavalue.value;
        switch (claim) {
          case "P2002":
          ...
          case "P159":
            ...
            try {
              postcode =
                res.claims[claim][0].qualifiers.P281[0].datavalue.value;
            } catch {}
            try {
              number = res.claims[claim][0].qualifiers.P670[0].datavalue.value;
            } catch {}

            try {
              lat =
                res.claims[claim][0].qualifiers.P625[0].datavalue.value
                  .latitude;
            } catch {}
            ...
            try {
              for (const str of res.claims[claim][0].qualifiers.P6375) {
            ...
            try {
              city = res.claims[claim][0].mainsnak.datavalue.value.id;
            } catch {}
          ...
          case "P2144":
            ...
            for (const freq of res.claims[claim]) {
              ...
              // MHz Q732707
              if (frequency >= 87.5 && frequency <= 108 && unit === "Q732707") {
                fm.push(frequency);
              } else if (
                frequency >= 531 &&
                frequency <= 1602 &&
